...
# Definir variables
key_name="ssh-mensagl-2025-borja"
...
vpc_id=$(aws ec2 describe-vpcs --filters "Name=tag:Name,Values=vpc-mensagl-2025-Borja" --query "Vpcs[0].VpcId" --output text)
subnet_public1_id=$(aws ec2 describe-subnets --filters "Name=tag:Name,Values=vpc-mensagl-2025-Borja-subnet-public1-us-east-1a" --query "Subnets[0].SubnetId" --output text)
subnet_public2_id=$(aws ec2 describe-subnets --filters "Name=tag:Name,Values=vpc-mensagl-2025-Borja-subnet-public2-us-east-1b" --query "Subnets[0].SubnetId" --output text)
subnet_private1_id=$(aws ec2 describe-subnets --filters "Name=tag:Name,Values=vpc-mensagl-2025-Borja-subnet-private1-us-east-1a" --query "Subnets[0].SubnetId" --output text)
subnet_private2_id=$(aws ec2 describe-subnets --filters "Name=tag:Name,Values=vpc-mensagl-2025-Borja-subnet-private2-us-east-1b" --query "Subnets[0].SubnetId" --output text)
...
aws ec2 run-instances --image-id "$ami_Ubuntu_24_04" --count 1 --instance-type "$instance_type" --key-name "$key_name" --security-group-ids "$sg_haproxy_id" --subnet-id "$subnet_public1_id" --private-ip-address "10.202.1.10" --associate-public-ip-address --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=HAProxyMatrix}]' --user-data file://script.sh
aws ec2 run-instances --image-id "$ami_Ubuntu_24_04" --count 1 --instance-type "$instance_type" --key-name "$key_name" --security-group-ids "$sg_haproxy_id" --subnet-id "$subnet_public2_id" --private-ip-address "10.202.2.10" --associate-public-ip-address--tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=HAProxyWordpress}]' --user-data file://script.sh

# Crear instancias Matrix-Synapse en subredes privadas
aws ec2 run-instances --image-id "$ami_Ubuntu_24_04" --count 1 --instance-type "$instance_type" --key-name "$key_name" --security-group-ids "$sg_matrix_synapse_id" --subnet-id "$subnet_private1_id" --private-ip-address "10.202.3.20" --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=Matrix-Synapse1}]' --user-data file://script.sh
aws ec2 run-instances --image-id "$ami_Ubuntu_24_04" --count 1 --instance-type "$instance_type" --key-name "$key_name" --security-group-ids "$sg_matrix_synapse_id" --subnet-id "$subnet_private2_id" --private-ip-address "10.202.3.21" --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=Matrix-Synapse2}]' --user-data file://script.sh

# Crear instancias Wordpress en subredes privadas
aws ec2 run-instances --image-id "$ami_Ubuntu_24_04" --count 1 --instance-type "$instance_type" --key-name "$key_name" --security-group-ids "$sg_wordpress_id" --subnet-id "$subnet_private1_id" --private-ip-address "10.202.4.20" --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=Wordpress1}]' --user-data file://script.sh
aws ec2 run-instances --image-id "$ami_Ubuntu_24_04" --count 1 --instance-type "$instance_type" --key-name "$key_name" --security-group-ids "$sg_wordpress_id" --subnet-id "$subnet_private2_id" --private-ip-address "10.202.4.21" --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=Wordpress2}]' --user-data file://script.sh

# Crear instancias PostgreSQL en subredes privadas
aws ec2 run-instances --image-id "$ami_Ubuntu_22_04" --count 1 --instance-type "$instance_type" --key-name "$key_name" --security-group-ids "$sg_postgres_id" --subnet-id "$subnet_private1_id" --private-ip-address "10.202.3.100" --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=Postgres1}]' --user-data file://script.sh
aws ec2 run-instances --image-id "$ami_Ubuntu_22_04" --count 1 --instance-type "$instance_type" --key-name "$key_name" --security-group-ids "$sg_postgres_id" --subnet-id "$subnet_private2_id" --private-ip-address "10.202.3.101" --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=Postgres2}]' --user-data file://script.sh
...
aws s3api create-bucket --bucket "s3-mensagl-borja" --region "$region"
...
aws ec2 run-instances \
    ...
    --private-ip-address "10.202.3.200" \
